...
void ObjectMgr::AddCreatureToGrid(uint32 guid, CreatureData const* data)
{
    ...
    for (uint8 i = 0; mask != 0; i++, mask >>= 1)
    {
        if (mask & 1)
        {
            CellCoord cellCoord = Arkcore::ComputeCellCoord(data->posX, data->posY);
    ...
    do
    {
        ...
        switch (tmp.command)
        {
            ...
            case SCRIPT_COMMAND_TELEPORT_TO:
            {
                ...
                if (!Arkcore::IsValidMapCoord(tmp.TeleportTo.DestX, tmp.TeleportTo.DestY, tmp.TeleportTo.DestZ, tmp.TeleportTo.Orientation))
                {
            ...
            {
                if (!Arkcore::IsValidMapCoord(tmp.TempSummonCreature.PosX, tmp.TempSummonCreature.PosY, tmp.TempSummonCreature.PosZ, tmp.TempSummonCreature.Orientation))
                {
...
void ObjectMgr::LoadPointsOfInterest()
{
    ...
    do
    {
        ...
        if (!Arkcore::IsValidMapCoord(POI.x, POI.y))
        {
...
bool ObjectMgr::LoadTrinityStrings(const char* table, int32 min_value, int32 max_value)
{
    ...
    if (!result)
    {
        if (min_value == MIN_arkcore_string_ID)              // error only in case internal strings
            sLog->outError(LOG_FILTER_SERVER_LOADING, ">>  Loaded 0 ARKCORE strings. DB table `%s` is empty. Cannot continue.", table);
        else
    ...
    if (min_value == MIN_arkcore_string_ID)
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u Arkcore strings from table %s in %u ms", count, table, GetMSTimeDiffToNow(oldMSTime));
    else
...
const char *ObjectMgr::GetTrinityString(int32 entry, LocaleConstant locale_idx) const
{
    ...
    if (entry > 0)
        ...
        sLog->outError(LOG_FILTER_SQL, "Arkcore string entry %i not found in DB.", entry);
    return "<error>";
}
...
bool LoadTrinityStrings(const char* table, int32 start_value, int32 end_value)
{
    ...
    {
        sLog->outError(LOG_FILTER_SQL, "Table '%s' load attempted with range (%d - %d) reserved by Arkcore, strings not loaded.", table, start_value, end_value+1);
        return false;
    }
